...
export const FullAnalysisReport = memo(function FullAnalysisReport({
  ...
  const cfToDist = data.latency_distribution?.filter((d) => d.direction === "cf_to_client") ?? [];
  const toCfDist = data.latency_distribution?.filter((d) => d.direction === "client_to_cf") ?? [];
  ...
  const hourlyRows = [...hourlyMap.entries()].sort((a, b) => a[0].localeCompare(b[0]));

  const bursts = detectErrorBursts(data.recent_errors);
  const probeErrors = data.probe_stats.filter((p) => p.status !== "ok");

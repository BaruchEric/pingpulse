...
/** Delete a client and all associated data (ping_results, speed_tests, client_probe_results, outages, alerts, traces). */
export async function deleteClientCascade(db: D1Database, id: string) {
  const results = await db.batch([
    ...
    db.prepare("DELETE FROM client_probe_results WHERE client_id = ?").bind(id),
    db.prepare("DELETE FROM outages WHERE client_id = ?").bind(id),
    db.prepare("DELETE FROM alerts WHERE client_id = ?").bind(id),
    db.prepare("DELETE FROM trace_hops WHERE trace_id IN (SELECT id FROM traces WHERE client_id = ?)").bind(id),
    db.prepare("DELETE FROM traces WHERE client_id = ?").bind(id),

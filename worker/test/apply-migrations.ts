...
const statements = [
  ...
  `CREATE TABLE IF NOT EXISTS speed_tests (id TEXT PRIMARY KEY, client_id TEXT NOT NULL, timestamp TEXT NOT NULL, type TEXT NOT NULL CHECK (type IN ('probe', 'full')), target TEXT NOT NULL DEFAULT 'worker' CHECK (target IN ('worker', 'edge')), download_mbps REAL NOT NULL, upload_mbps REAL NOT NULL, payload_bytes INTEGER NOT NULL, duration_ms INTEGER NOT NULL, FOREIGN KEY (client_id) REFERENCES clients(id) ON DELETE CASCADE)`,
  `CREATE INDEX IF NOT EXISTS idx_speed_tests_client_ts ON speed_tests(client_id, timestamp)`,
  `CREATE TABLE IF NOT EXISTS outages (id TEXT PRIMARY KEY, client_id TEXT NOT NULL, start_ts TEXT NOT NULL, end_ts TEXT, duration_s REAL, FOREIGN KEY (client_id) REFERENCES clients(id) ON DELETE CASCADE)`,
  ...
  `CREATE TABLE IF NOT EXISTS trace_hops (id INTEGER PRIMARY KEY AUTOINCREMENT, trace_id TEXT NOT NULL, flow_id INTEGER NOT NULL DEFAULT 0, ttl INTEGER NOT NULL, addr TEXT, hostname TEXT, asn INTEGER, asn_name TEXT, geo TEXT, loss_pct REAL, samples INTEGER, last_ms REAL, avg_ms REAL, best_ms REAL, worst_ms REAL, stddev_ms REAL, jitter_ms REAL, FOREIGN KEY (trace_id) REFERENCES traces(id) ON DELETE CASCADE)`,

...
1. User runs `pingpulse register --token <TOKEN>` (token generated from dashboard)
2. Client sends token to API Worker, receives a client ID + WebSocket URL
3. Client stores credentials locally (`~/.pingpulse/config.toml`)
...
- Maintain WebSocket connection to its Durable Object (auto-reconnect with exponential backoff)
...
- Calculates: RTT, jitter (std deviation of recent RTTs), packet loss (missed echoes / total)
- Writes results to D1 + Analytics Engine
- Checks thresholds, triggers alert if breached
- Sets next alarm
...
|---|---|---|
| `/api/auth/register` | POST | Exchange token for client ID + WS URL |
...
| `/api/speedtest/:id` | POST | Trigger on-demand full speed test |
...
- `clients` — `id, name, location, token_hash, config_json, created_at, last_seen`
...
**Tech stack:** React + Vite, Cloudflare Workers Static Assets, Tailwind CSS, dark mode default.
...
- **Email:** Cloudflare email Worker binding (MailChannels) or transactional email API (Resend/Mailgun). Single admin address.
...
│   ├── Cargo.toml
│   ├── src/
│   │   ├── main.rs           # CLI entry (register, start, status, stop)
